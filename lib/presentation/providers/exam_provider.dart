  ...
  Future<void> loadQuestions() async {
    ...
    try {
      ...
      for (final level in levels) {
        ...
        final ids = await PerfTrace.timeAsync('exam.fetchIds',
            () => _wordRepo.fetchWordIds(language: questionLang, level: level));
        if (ids.isEmpty) continue;
        ids.shuffle(rng);
        allQuestionIds.addAll(
            ids.take(AppConstants.questionsPerLevel));
      }

      if (allQuestionIds.isEmpty) {
        state = state.copyWith(isLoading: false, errorMessage: 'No words found');
        return;
      }

      // ── Phase 2: batch-fetch question + answer words ──
      final questionWords = await PerfTrace.timeAsync('exam.fetchQuestions',
          () => _wordRepo.fetchWordsByIds(questionLang, allQuestionIds));
      final answerWords = await PerfTrace.timeAsync('exam.fetchAnswers',
      ...
      for (final level in levels) {
        final ids = await _wordRepo.fetchWordIds(
            language: answerLang, level: level);
        ids.shuffle(rng);
      ...
      for (final id in allQuestionIds) {
        ...
        final candidates =
            globalPool.where((w) => w != correctAnswer).toList();
        candidates.shuffle(rng);
        final distractors =
            candidates.take(AppConstants.distractorsPerQuestion).toList();
